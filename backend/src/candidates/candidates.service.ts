...
export class CandidatesService {
  ...
  async getJobCandidates(jobId: string) {
    ...
    const result = students.map((student) => {
      ...
      const gpaScore = job.minGpa
        ? Math.min((gpa / job.minGpa) * 100, 100)
        : 100;
      ...
      return {
        ...
        ranking: student.Ranking[0]?.score ?? 0,
      };
    });
  ...
  async analyzeCandidates(jobId: string) {
    const candidates = await this.getJobCandidates(jobId);
