...
export class RecommendationsService {
    ...
    async getStudentRecommendations(studentId: number) {
        ...
        const result = jobs.map(job => {
            ...
            const requiredSkills =
                job.JobSkill.map(item => item.skillId);
            ...
            const experienceScore =
                job.minExperience
                    ? Math.min(
                        (experience / job.minExperience) * 100,
                        100
                    )
                    :
                    100;
    ...
    async aiRecommendation(studentId:number){

        const recommendations =
            await this.getStudentRecommendations(studentId);
        ...
        const data = {
            student:{
                ...
                experience:
                student?.Profile?.experienceMonths,
